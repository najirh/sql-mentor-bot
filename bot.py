...
# Replace the single CHANNEL_ID with a list of channel IDs
...
async def get_question(difficulty=None):
    async with bot.db.acquire() as conn:
        if difficulty:
            question = await conn.fetchrow('''
                SELECT q.* FROM questions q
                LEFT JOIN reports r ON q.id = r.question_id
                WHERE q.difficulty = $1 AND r.question_id IS NULL
...
async def update_weekly_points(username, points):
    async with bot.db.acquire() as conn:
        week_start = await get_week_start()
        await conn.execute('''
            INSERT INTO weekly_points (username, points, week_start)
            VALUES ($1, $2, $3)
            ON CONFLICT (username, week_start)
            DO UPDATE SET points = weekly_points.points + EXCLUDED.points
        ''', username, points, week_start)

async def get_weekly_points(username):
    async with bot.db.acquire() as conn:
        week_start = await get_week_start()
        points = await conn.fetchval('''
            SELECT COALESCE(points, 0)
            FROM weekly_points
            WHERE username = $1 AND week_start = $2
        ''', username, week_start)
    return points
...
async def on_ready():
    ...
    await wait_for_db()
    await start_healthcheck()  # Add this line to start the health check server
...
async def get_difficulty_question(ctx, difficulty):
    global current_question
    try:
        current_question = await get_question(difficulty)
        if current_question:
            await display_question(ctx, current_question)
        else:
...
async def my_scores(ctx):
    try:
        async with bot.db.acquire() as conn:
            total_points = await conn.fetchval('SELECT points FROM leaderboard WHERE username = $1', str(ctx.author))
            weekly_points = await get_weekly_points(str(ctx.author))
            streak = await get_user_streak(str(ctx.author))
...
async def submit(ctx, *, answer):
    ...
    try:
        async with bot.db.acquire() as conn:
            ...
            await conn.execute('''
                INSERT INTO user_submissions (username, question_id, points)
                VALUES ($1, $2, $3)
            ''', str(ctx.author), current_question['id'], points)

            # Update weekly_points
            await update_weekly_points(str(ctx.author), points)
            ...
            if not correct and (str(ctx.author) not in user_attempts or user_attempts[str(ctx.author)] == 0):
                user_attempts[str(ctx.author)] = 1
                ...
                user_attempts[str(ctx.author)] = 0
        ...
        user_skips[str(ctx.author.id)] = False
...
async def try_again(ctx):
    if str(ctx.author) in user_attempts and user_attempts[str(ctx.author)] == 1:
...
async def top_10(ctx):
    try:
        async with bot.db.acquire() as conn:
            top_users = await conn.fetch('SELECT username, points FROM leaderboard ORDER BY points DESC LIMIT 10')
        
        if top_users:
            headers = ["Rank", "Username", "Points"]
    ...
    try:
        async with bot.db.acquire() as conn:
            ...
            top_users = await conn.fetch('''
                SELECT username, COUNT(*) as submissions, SUM(points) as points
                FROM user_submissions
        ...
        if top_users:
            headers = ["Rank", "Username", "Submissions", "Points", "Streak"]
            data = []
            for i, user in enumerate(top_users, 1):
                streak = await get_user_streak(user['username'])
    ...
    try:
        async with bot.db.acquire() as conn:
            ...
            await conn.execute('''
                ...
                VALUES ($1, $2, $3)
            ''', str(ctx.author), question_id, feedback)
...
async def skip(ctx):
    ...
    try:
        new_question = await get_question()
        if new_question:
